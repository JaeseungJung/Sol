...
contract D-Research is ERC1155 {
   ...
   struct r_Info{
        uint eth_Reward;
        uint nft_Reward;
        mapping(address => uint) private is_Applied;
    }
    ...
    function create_Research(uint _r_Id, uint _eth_Reward, uint _nft_Reward) public onlyAdmin  { 
        research_Info[r_Id].eth_Reward = _eth_Reward;
        research_Info[r_Id]._nft_Reward = _nft_Reward;
   }
    ...
    function edit_Research(uint _r_Id, uint _eth_Reward, uint _nft_Reward) public onlyAdmin  { 
     ...
     function apply_Research(uint _r_Id) public  { 
        research_Info[r_Id].is_Applied[msg.sender] = 1;
   }

   function VC_Create(uint _vc_Id, string _uri) public { 
   ...
   function VC_Delete() public onlyMember { 
        user_To_Vc[msg.sender].vc1 = 0;
        user_To_Vc[msg.sender].vc2 = 0;
        user_To_Vc[msg.sender].vc3 = 0;
   }


    function Get_Reward(uint _r_Id) public onlyMember { 
        isApplied = research_Info[r_Id].is_Applied[msg.sender];
        require(isApplied == 1, "not applied to research");

        uint get_Eth_amount = research_Info[r_Id].eth_Reward;
        payable(msg.sender).transfer(get_Eth_amount);

        uint get_Nft_Id = research_Info[r_Id].nft_Reward;
    ...
    modifier onlyAdmin () {
     ...
     modifier onlyMember () {
        is_Member = user_To_Vc[msg.sender].is_Member;
